...
contract Keep3rV1 is ReentrancyGuard {
    ...
    function workReceipt(address keeper, uint amount) public {
        ...
        require(amount <= KPRH.getQuoteLimit(_gasUsed.sub(gasleft())));
        credits[msg.sender][address(this)] = credits[msg.sender][address(this)].sub(amount);
        lastJob[keeper] = now;
    ...
    function receipt(address credit, address keeper, uint amount) external {
        ...
        credits[msg.sender][credit] = credits[msg.sender][credit].sub(amount);
        lastJob[keeper] = now;
    ...
    function receiptETH(address keeper, uint amount) external {
        require(jobs[msg.sender], "receipt: !job");
        credits[msg.sender][ETH] = credits[msg.sender][ETH].sub(amount);
        lastJob[keeper] = now;
